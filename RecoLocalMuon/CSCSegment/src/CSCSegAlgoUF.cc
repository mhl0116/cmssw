...
CSCSegAlgoUF::CSCSegAlgoUF(const edm::ParameterSet& ps) : 
  CSCSegmentAlgorithm(ps), myName_("CSCSegAlgoUF"), ps_(ps),  showering_(0) {
  ...
  if (debug) edm::LogVerbatim("CSCSegment") << "CSCSegAlgoUF: with factored conditioned segment fit";
}
...
std::vector<CSCSegment> CSCSegAlgoUF::run(const CSCChamber* aChamber, 
  ...
  LogTrace("CSCSegAlgoUF") << "[CSCSegAlgoUF::run] Start building segments in chamber " << theChamber->id();
...
// ********************************************************************;
std::vector< std::vector<const CSCRecHit2D*> > CSCSegAlgoUF::clusterHits(const CSCChamber* aChamber, const ChamberHitContainer & rechits) {
  theChamber = aChamber; 
  ...
  for(size_t NNN = 0; NNN < seeds.size(); ++NNN) {
	...
	LogTrace("CSCSegment|CSC") << "[CSCSegAlgoUF::clusterHits] ALARM! Skipping used seeds, this should not happen - inform CSC DPG";
...
std::vector< std::vector<const CSCRecHit2D*> > CSCSegAlgoUF::chainHits(const CSCChamber* aChamber, const ChamberHitContainer & rechits) {
...
bool CSCSegAlgoUF::isGoodToMerge(bool gangedME11a, ChamberHitContainer & newChain, ChamberHitContainer & oldChain) {
...
double CSCSegAlgoUF::theWeight(double coordinate_1, double coordinate_2, double coordinate_3, float layer_1, float layer_2, float layer_3) {
  double sub_weight = 0;
  sub_weight = fabs( 
 ...
 */
std::vector<CSCSegment> CSCSegAlgoUF::buildSegments(const ChamberHitContainer& rechits) {
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++  
    ...
    //  LogTrace("CSCSegment|CSC") << "[CSCSegAlgoUF::buildSegments] No. of rechits in the cluster/chamber > " 
    //  << UpperLimit << " ... Segment finding in the cluster/chamber canceled!";
      CSCDetId id = rechits[0]->cscDetId();
      edm::LogVerbatim("CSCSegment|CSC") << "[CSCSegAlgoUF::buildSegments] No. of rechits in the cluster/chamber > " 
  ...
  case 4 : 
    ...
    if ((best_Layer_noLx < 1) || (best_Layer_noLx > 4)) {
      //      std::cout<<"CSCSegAlgoUF: For four layers, best_Layer_noLx = "<< best_Layer_noLx << std::endl;
    }
  ...
  case 5 : 
    ...
    if ((best_Layer_noLx < 1) || (best_Layer_noLx > 5)) {
      //      std::cout<<"CSCSegAlgoUF: For five layers, best_Layer_noLx = "<< best_Layer_noLx << std::endl;
    }
  ...
  case 6 : 
    ...
    if ((best_Layer_noLx < 1) || (best_Layer_noLx > 6)) {
      //      std::cout<<"CSCSegAlgoUF: For six layers, best_Layer_noLx = "<< best_Layer_noLx << std::endl;
    }
  ...
  default : 
    ...
    LogTrace("CSCSegment|CSC") << "[CSCSegAlgoUF::buildSegments] Unexpected number of layers with hits - please inform CSC DPG.";
    hit_drop_limit = 0.1;
  }
  ...
  for(unsigned int iSegment=0; iSegment<GoodSegments.size();++iSegment){
	...
	segfit->fit(condpass1, condpass2);
      ...
      if(segfit->scaleXError() < 1.00005){
        LogTrace("CSCWeirdSegment") << "[CSCSegAlgoUF::buildSegments] Segment ErrXX scaled and refit " << std::endl;
	if(segfit->chi2()/segfit->ndof()>chi2Norm_3D_){
     ...
     // Achieved by decreasing abs value of the Covariance
          LogTrace("CSCWeirdSegment") << "[CSCSegAlgoUF::buildSegments] Segment ErrXY changed to match cond. number and refit " << std::endl;
	  condpass2 = true;
	  segfit->fit(condpass1, condpass2);
        }
      }
	 ...
	 (segfit->nhits()>3)){   
        LogTrace("CSCWeirdSegment") << "[CSCSegAlgoUF::buildSegments] Scale factor chi2uCorrection too big, pre-Prune and refit " << std::endl;
...
void CSCSegAlgoUF::ChooseSegments2a(std::vector< ChamberHitContainer > & chosen_segments, int chosen_seg) {
  // just return best segment
  GoodSegments.clear();
  GoodSegments.push_back( chosen_segments[chosen_seg] );
}

void CSCSegAlgoUF::ChooseSegments3(std::vector< ChamberHitContainer > & chosen_segments, std::vector< float > & chosen_weight, int chosen_seg) {
...
void CSCSegAlgoUF::ChooseSegments2(int best_seg) {
  ...
  for(unsigned int iCand=0;iCand<Psegments.size();++iCand) {
	...
	LogTrace("CSCSegment|CSC") << "[CSCSegAlgoUF::ChooseSegments2] ALARM!! Should not happen - please inform CSC DPG";
      }
      else {
...
void CSCSegAlgoUF::findDuplicates(std::vector<CSCSegment>  & segments ){
...
void CSCSegAlgoUF::dumpSegment( const CSCSegment& seg ) const {
